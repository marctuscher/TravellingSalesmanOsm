					...
					case "APX":
						...
						axios({
							...
							store.dispatch(__WEBPACK_IMPORTED_MODULE_0__actions__["createNotification"]({
								...
								sticky: true,
								appState: "tsp",
								content: '<h6 style="color:#44bd32">Calculation Times:</h6></br>' + 'Finding Nodes: ' + getSecondsString(res.data["duration:localization"]) + '</br>' + 'Dijkstra: ' + getSecondsString(res.data["duration:dijkstra"]) + '</br>' + 'TSP: ' + getSecondsString(res.data["duration:compute"]) + '</br>' + 'Costs before swap: ' + res.data.costsBefore + '</br>' + 'Costs: ' + res.data.costs
							}));
							action = processResult(res, action, store);
							next(action);
						}).catch(function (err) {
...
//# sourceMappingURL=0.29c15a3dfd87047ae9bf.hot-update.js.map